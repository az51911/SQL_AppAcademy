...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    SELECT first_leg.num, first_leg.company, second_leg.name, second_leg.num, second_leg.company
    FROM
      (SELECT
        stopb.name AS name, a.company AS company, a.num AS num
      FROM
        routes a
      JOIN
        routes b ON (a.company = b.company AND a.num = b.num)
      ...
      INNER JOIN
      (SELECT
        stopc.name AS name, c.company AS company, c.num AS num
      FROM
        routes c
      JOIN
      ...
      GROUP BY
        stopc.name, c.company, c.num
      ) AS second_leg
      ON
        first_leg.name = second_leg.name
      WHERE
        first_leg.company != second_leg.company OR first_leg.num != second_leg.num
  SQL
end
